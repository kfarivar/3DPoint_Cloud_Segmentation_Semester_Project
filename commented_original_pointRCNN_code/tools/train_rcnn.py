...
from tensorboardX import SummaryWriter # for logging during training of network
import os
import argparse
import logging
...
def create_dataloader(logger):
    ...
    train_set = KittiRCNNDataset(root_dir=DATA_PATH, npoints=cfg.RPN.NUM_POINTS, split=cfg.TRAIN.SPLIT, mode='TRAIN',
                                 ...
                                 gt_database_dir=args.gt_database) # this is the pickled database we made using generate_gt_database.py
...
if __name__ == "__main__":
    ...
    ''' NOTE:  This file is used for both training the first (rpn) and the second (rcnn) networks.
    '''


    # read configuration from file (if a config not given default values used)
    if args.cfg_file is not None:
        cfg_from_file(args.cfg_file)
    # TAG is the name of the config file without the extension (e.g. for "cfgs/default.yaml" TAG is default)
    cfg.TAG = os.path.splitext(os.path.basename(args.cfg_file))[0] 

    # rpn is the first network
    ...
    # next lines are about logging the GPUs and arguments received 
    ...
    # logs the values of the config
    save_config_to_file(cfg, logger=logger)

    # copy important files to backup (no idea why this is useful ?!)
    ...
    # load checkpoint if it is possible (apparently you can resume optimization progress from a checkpoint)
    start_epoch = it = 0
    last_epoch = -1
    ...
    # if we give this function a 'rpn_ckpt' then it most probably means we are training the rcnn (second network)
    if args.rpn_ckpt is not None:
        pure_model = model.module if isinstance(model, torch.nn.DataParallel) else model
        total_keys = pure_model.state_dict().keys().__len__()
        #  this probably just loads the previously trained rpn
