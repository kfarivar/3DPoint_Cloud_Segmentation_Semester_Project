...
def get_reg_loss(pred_reg, reg_label, loc_scope, loc_bin_size, num_head_bin, anchor_size,
    ...
    :param pred_reg: (N, C) because we do bin based regression there is more than 7 outputs predicted
    ...
    # offset is used to determin which heads of the reg networks are for which bin based classification
    # pred_reg[:, x_bin_l: x_bin_r] are the classification heads for x
    ...
    # we not only subtract by mean we also divide by the mean 
