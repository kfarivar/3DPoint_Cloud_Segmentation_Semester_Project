...
def boxes3d_to_corners3d(boxes3d, rotate=True):
    """
    Finds the 8 cornesr of the all the N boxes in the input
    :param boxes3d: (N, 7) [x, y, z, h, w, l, ry]  (ry the rotation is assuming x,y,z as origin !)
    :param rotate:
    :return: corners3d: (N, 8, 3) the 8 corners of the N boxes 
    """
    boxes_num = boxes3d.shape[0] # N
    ...
    # so why is the convention for y different ?
    # this gives as first 4 corners 0 and the last 4 -h 
    ...
    if rotate:
        ...
        # this is just the famous rotation matrix in the x-z plane
    ...
    # ry the rotation is assuming x,y,z as origin ! that is why we first rotate the dimensions
    # then add the x,y,z values
    x = x_loc.reshape(-1, 1) + x_corners.reshape(-1, 8)
    y = y_loc.reshape(-1, 1) + y_corners.reshape(-1, 8)
    z = z_loc.reshape(-1, 1) + z_corners.reshape(-1, 8)

    # each box (N many) has 8 cornesr with x,y,z
...
def enlarge_box3d(boxes3d, extra_width):
    ...
    # this is probably related to the fact that y is differently defined compared to x and y
    # see boxes3d_to_corners3d_torch above
...
def in_hull(p, hull):
    ...
    try:
        if not isinstance(hull, Delaunay):
            hull = Delaunay(hull)
        # hull.find_simplex(p) looks to give points inside convex hull of the box corners value + and outside -
...
def objs_to_boxes3d(obj_list):
    '''return a list of bounding box properties of each object in input (obj_list) '''
