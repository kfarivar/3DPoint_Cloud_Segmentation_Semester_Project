...
class KittiRCNNDataset(KittiDataset):
    ...
    def __init__(self, root_dir, npoints=16384, split='train', classes='Car', mode='TRAIN', random_select=True,
        ...
        if cfg.RPN.ENABLED:
            if gt_database_dir is not None:
                ...
                # a list of objects (e.g. cars) in all the scenes. each object is defined as a dictionary of attributes
                self.gt_database = pickle.load(open(gt_database_dir, 'rb'))

                # No idea what the value of cfg.GT_AUG_HARD_RATIO means
                # the whole point of this if block looks to be making the gt_database list (easy and hard)
                if cfg.GT_AUG_HARD_RATIO > 0:
                    easy_list, hard_list = [], []
                    # for each object (e.g. Car) in the database
                    for k in range(self.gt_database.__len__()):
                        obj = self.gt_database[k]
                        # if the object has more than 100 points it is easy
                        if obj['points'].shape[0] > 100:
                            easy_list.append(obj)
                        else:
                            hard_list.append(obj)
                    # here we divide the augmentation database into easy and hard examples        
            ...
            if mode == 'TRAIN':
                # this just makes the 'sample_id_list' a list of ids of samples(scenes) where we have objects of interest (e.g. Cars)
    ...
    def preprocess_rpn_training_data(self):
        ...
        # label_dir defined in parent class (the original labeling of data)
        self.logger.info('Loading %s samples from %s ...' % (self.mode, self.label_dir)) 
        
        # the puspose of this loop seems to be just getting the ids of the scenes; 
        # where there exists at least one object of interest (e.g. Car) saving them in 'sample_id_list'.
        # 'num_sample' is the number of scenes in training set(defined in parent class)
        for idx in range(0, self.num_sample):
            sample_id = int(self.image_idx_list[idx]) # 'image_idx_list' the list of ids of scenes for training

            # first get a list of objects (with their properties like boundingbox and points) in the scene using the label data 
            # then only pick (filter) those that are in the  self.classes (e.g. Car)
            obj_list = self.filtrate_objects(self.get_label(sample_id)) # note 
            if len(obj_list) == 0:
    ...
    def get_label(self, idx):
        '''returns a list of objects in the scene (label file given)
           each has it's attributes like bounding box measures and etc
        '''

        # I have no idea what this check is about (seems to be related to augmentation)
        # since we only have around 7 thousand and something scenes
        if idx < 10000:
    ...
    def get_valid_flag(pts_rect, pts_img, pts_rect_depth, img_shape):
        ...
        # why would we restrict the points in a specific range ?
...
# __len__ and __getitem__ are two functions we need to define to make this a real torch_data.Dataset class
# __len__ so that len(dataset) returns the size of the dataset.
#__getitem__ to support the indexing such that dataset[i] can be used to get i-th sample
 
    def __len__(self):
        if cfg.RPN.ENABLED:
            # we only use scenes where there is a car
    ...
    # get 1 scene (sample) using the given index
    def get_rpn_sample(self, index):

        # get the real sample index 
        ...
        if sample_id < 10000:
            ...
            # get valid point (projected points should be in image) (meaning transform from lidar coords to image coords)
        ...
        # only get valid points and intensities (Valid point should be in the image (and in the PC_AREA_SCOPE) (a range for x,y,z I have no idea about))
        pts_rect = pts_rect[pts_valid_flag][:, 0:3]
        pts_intensity = pts_intensity[pts_valid_flag]

        if cfg.GT_AUG_ENABLED and self.mode == 'TRAIN':
            # all labels for checking overlapping
            all_gt_obj_list = self.filtrate_dc_objects(self.get_label(sample_id)) # remove don't care objects from the list of objects
            all_gt_boxes3d = kitti_utils.objs_to_boxes3d(all_gt_obj_list) # get a list of all bounding boxes from this scene

            gt_aug_flag = False
            if np.random.rand() < cfg.GT_AUG_APPLY_PROB: # decide if we are going to apply augmentation to this scene
                # augment one scene
                # if no augmentation happens then gt_aug_flag = False else True
                # if there is an augmentation the new objects added 
                # have their points and intensifies added to the  pts_rect, pts_intensity (overlaping points are removed)
                # extra_gt_boxes3d: is a list of bboxes of augmented objects 
                # extra_gt_obj_list: a list of augmented objects
        ...
        if self.mode == 'TRAIN' or self.random_select:
            # this looks like sampling 
            # we sample if the number of points we have (len(pts_rect)) is more than we thershold set in config file (npoints)
            if self.npoints < len(pts_rect):
                # separate far points (higher in z dim) and include all since lidar resolution is low when objects are far
                # then fill the capaity left (self.npoints - len(far_idxs_choice)) with sampled near points 
                pts_depth = pts_rect[:, 2]
                pts_near_flag = pts_depth < 40.0
                #np.where(condition) is shorthand for np.asarray(condition).nonzero()
                #numpy.nonzero: Returns a tuple of arrays, one for each dimension of a, 
                # containing the indices of the non-zero elements in that dimension.
                far_idxs_choice = np.where(pts_near_flag == 0)[0]
                near_idxs = np.where(pts_near_flag == 1)[0]
                # make a random sample of elements in near_idxs without replacement of size = self.npoints - len(far_idxs_choice)
                near_idxs_choice = np.random.choice(near_idxs, self.npoints - len(far_idxs_choice), replace=False)

                # merge far and near choices
                choice = np.concatenate((near_idxs_choice, far_idxs_choice), axis=0) \
                    if len(far_idxs_choice) > 0 else near_idxs_choice
                # why do we shuffle indexes?!
                np.random.shuffle(choice)
            else:
                choice = np.arange(0, len(pts_rect), dtype=np.int32)
                if self.npoints > len(pts_rect): # they are not equal
                    # how is this helpful ? (multiple copies of the same point ?) 
        ...
        # this line doesn't make any sense !? by the line above pts_features has always exactly 1 element
        # in what case len(pts_features) > 1 ?
        ...
        if cfg.GT_AUG_ENABLED and self.mode == 'TRAIN' and gt_aug_flag:
            # add the augmented objects to this scenes object list
        ...
        # data augmentation (this time it is about 'rotation', 'scaling', 'flip')
        ...
        # the rpn_cls_label is the classification label for the classification head of RPN network
        # the rpn_reg_label is the regression targets: for each point inside a bbox, that point has the values of the
        # bbox properties  dx, dy, dz, ry, h, w, l as a tagert for the regression. 
        # for the bbox regression head of the RPN network
    ...
    def generate_rpn_training_labels(pts_rect, gt_boxes3d):
        '''sets the class of each point inside the original bbox to 1
           The points included after enlarging the bbox have class -1
           other points 0

           for regression target it uses the same h, w, l, ry as the bbox for each point 
           inside the original bbox, but the x,y,z are transformed into dx, dy, dz (not sure what this is ?!)

           note: gt_boxes3d in each row has: obj.pos, obj.h, obj.w, obj.l, obj.ry
        '''

        cls_label = np.zeros((pts_rect.shape[0]), dtype=np.int32)
        reg_label = np.zeros((pts_rect.shape[0], 7), dtype=np.float32)  # dx, dy, dz, h, w, l, ry
        ...
        for k in range(gt_boxes3d.shape[0]):
            ...
            # select points that are in the enlarged box but not in the original box
            ignore_flag = np.logical_xor(fg_pt_flag, fg_enlarge_flag)
            cls_label[ignore_flag] = -1

            # pixel offset of object center
            center3d = gt_boxes3d[k][0:3].copy()  # (x, y, z)
            # I don't know what this is ? I guess since there was this diff between a,z and y he is trying to correct that
            # see the boxes3d_to_corners3d_torch in kitti_utils.py to see the diff
            center3d[1] -= gt_boxes3d[k][3] / 2 
            
            # center all points around the box center 
            # new coordinate system (I think !) 
    ...
    '''
    each object in the scene has this attributes
    ...
    def apply_gt_aug_to_one_scene(self, sample_id, pts_rect, pts_intensity, all_gt_boxes3d):
        ...
        :param pts_rect: the points of the whole scene (N, 3)
        :param all_gt_boxex3d: list of bounding boxes of all the objects of interest in the scene (M2, 7)
        :return:
        """
        ...
        # order of the box info pos(x,y,z), h, w, l, ry (angle)
        cur_gt_boxes3d = all_gt_boxes3d.copy()
        cur_gt_boxes3d[:, 4] += 0.5  # TODO: consider different objects (width of the box)
        cur_gt_boxes3d[:, 5] += 0.5  # enlarge new added box to avoid too nearby boxes (length)
        cur_gt_corners = kitti_utils.boxes3d_to_corners3d(cur_gt_boxes3d) # get the 8 corners of each box

        extra_gt_obj_list = []
        extra_gt_boxes3d_list = []
        new_pts_list, new_pts_intensity_list = [], []
        # this is a list that tells use which points are overlaping the augmentation object so we can remove them
        src_pts_flag = np.ones(pts_rect.shape[0], dtype=np.int32)

        road_plane = self.get_road_plane(sample_id)
        a, b, c, d = road_plane

        # either we run out of time (try_times) or we get the number of augmentations 
        # we asked for (cnt > extra_gt_num)
        while try_times > 0:
            if cnt > extra_gt_num:
                break
            ...
            # remember we divided the augmentation database into easy and hard samples (in init)
            # here we decide if we want to add easy or hard object
            if cfg.GT_AUG_HARD_RATIO > 0:
                p = np.random.rand()
            ...
            # also remember if we don't have cfg.GT_AUG_HARD_RATIO > 0
            # we didn't divide the gt_database into easy and hard so just
            # pick sth randomly
            else:
                rand_idx = np.random.randint(0, self.gt_database.__len__())
                new_gt_dict = self.gt_database[rand_idx]

            # now we have the id of the random object in rand_idx
            # and the object features in new_gt_dict

            # get the bbox
            new_gt_box3d = new_gt_dict['gt_box3d'].copy() # get the bbox
            new_gt_points = new_gt_dict['points'].copy() # lidar points
            new_gt_intensity = new_gt_dict['intensity'].copy()
            new_gt_obj = new_gt_dict['obj'] # this is the object(programming) of the object in the scene that contains all properties of this object
            center = new_gt_box3d[0:3]
            # check if the center of object to augment is in the range defined by __C.PC_AREA_SCOPE
            ...
            new_gt_obj.pos[1] -= move_height # update the same y in this object
            ...
            # note we previously enlarged the bboxes of every object in the current original
            # scene we want to return, but here we enlarge the bbox of the object to augment

            cnt += 1
            new_corners = kitti_utils.boxes3d_to_corners3d(new_enlarged_box3d.reshape(1, 7))
            # calculate the intersection over union of the data to be augmented and all the other
            # objects in the current original scene
            iou3d = kitti_utils.get_iou3d(new_corners, cur_gt_corners)
            # if no major overlap go ahead and add it to the scene
            ...
            # make a matrix showing which point are inside the bbox of the object we want to augment
            # so we can remove them (since we have 1 bbox returns 1 by #points matrix)
            ...
            # add the new augmented object to the lists
            ...
            # keep a separate record of the bboxes of new augmented objects
    ...
    def data_augmentation(self, aug_pts_rect, aug_gt_boxes3d, gt_alpha, sample_id=None, mustaug=False, stage=1):
        """
        data augmentations like 'rotation', 'scaling', 'flip'
        ...
        # we map the prob from [0,1) to (0,1]
        # this determins the prob of augmenting or not for each type of augmentation
        ...
        if 'rotation' in aug_list and aug_enable[0] < cfg.AUG_METHOD_PROB[0]:
            ...
            # we rotate all point cloud (pc) in a scene along y (keeping the y variables constant)
            aug_pts_rect = kitti_utils.rotate_pc_along_y(aug_pts_rect, rot_angle=angle)
            if stage == 1:
                # xz change, y & hwl unchanged
                ...
                # beta is the angel between the positive x axis and line passing through origin and x,z
                # beta is not the same as the new_ry. ry is in the coordinates of the center of bbox
                beta = np.arctan2(z, x)
    ...
    def collate_batch(self, batch):
        '''
        we need to implement this since atleast for RPN we have a different number of bboxes in each scene
        but since we have an euqal number of pionts the rest of the features of the scene are simpler to batch
        '''
        ...
        for key in batch[0].keys():
            if cfg.RPN.ENABLED and key == 'gt_boxes3d' or \
                ...
                # put all the batch scenes' bboxes(bbox features) in a single array 
