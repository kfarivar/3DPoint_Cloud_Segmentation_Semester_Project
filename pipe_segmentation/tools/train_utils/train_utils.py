...
class Trainer(object):
    ...
    def _train_it(self, batch):
        ...
        # model_fn is in /lib/net/train_functions.py 
        # it calculates the loss
        # disp_dict is just a dictionary with loss in it for printing
        # no idea what tb_dict it seems to be just empty
        loss, tb_dict, disp_dict = self.model_fn(self.model, batch)

        
        loss.backward()

        # if the norm of the gradients of all parameters is more than self.grad_norm_clip 
        # it normalizes the gradient vector (*max/our_norm) so the norm becomes at most self.grad_norm_clip  
    ...
    def train(self, start_it, start_epoch, n_epochs, train_loader, test_loader=None, ckpt_save_interval=5,
        ...
        # save a list of losses for each epoch
        train_losses = []
        val_losses = []
        ...
        with tqdm.trange(start_epoch, n_epochs, desc='epochs') as tbar, \
            ...
            for epoch in tbar:
                ...
                # save the loss calculated in one epoch
                cur_loss = count = 0.0
                ...
                # add the cur_loss to the train_losses list
                # we normalize by the number of batches like in the eval_epoch() above
                train_losses.append(cur_loss/count)
                ...
                if (epoch % eval_frequency) == 0:
                    ...
                    if test_loader is not None:
                        with torch.set_grad_enabled(False):
                            ...
                            # add the validation loss to the val_losses
