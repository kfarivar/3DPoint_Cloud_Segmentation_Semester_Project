...
def model_joint_fn_decorator():
    ...
    def model_fn(model, data):
        # note the data is a batch the way we defined it in "collate_batch" in lib/datasets/pipes_rcnn_dataset.py 

        if cfg.RPN.ENABLED:
            # pts_rect and pts_input are the same
            # pts_features is all 0s (it is not used !)
            pts_rect, pts_features, pts_input = data['pts_rect'], data['pts_features'], data['pts_input']
            # D np array: x,y,z, w,h,l , rx,ry,rz, class_num for each box (exclude class number)
            gt_boxes3d = data['gt_boxes3d'][:,0:-1]

            # cast everything to torch tensor
            ...
            # gt_boxes3d is part of the input but is never used in the RPN network
        ...
        # the model is just: lib/net/rpn.py
